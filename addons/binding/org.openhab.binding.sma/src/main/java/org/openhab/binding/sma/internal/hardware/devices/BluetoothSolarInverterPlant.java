			...
			for (int i = 0; i < inverters.size(); i++) {
				...
				if (current != null) {
					SmaSerial serial = new SmaSerial(
					        (short)AbstractPhysicalLayer.getShort(data, 55),
	                ...
	                logger.debug(
	                    "Unexpected response from {} -> ", address);
	            }
			}
...
//		layer.open();
			...
			for (int i = 0; i < inverters.size(); i++) {
				...
				if ((pcktID == rcvpcktID) && (AbstractPhysicalLayer.getInt(data, 41) == now)) {
					BluetoothSolarInverterPlant.Data current = this.invertersByAddress.get(address
							.toString());
					if (current != null) {
						current.setSerial(new SmaSerial((short)AbstractPhysicalLayer.getShort(
								data, 15), AbstractPhysicalLayer.getInt(data, 17)));
	...
	public void setInverterTime() throws IOException
	{
        ...
        do
        {
            ...
            layer.writePacket((byte) 0x10, (byte) 0xA0, (short) 0,
                    PhysicalLayer.ANYSUSYID,
                    PhysicalLayer.ANYSERIAL, pcktID);
...
//			layer.open();
			...
			do {
				...
				layer.writePacket((byte) 0x09, (byte) 0xA0, (short) 0,
						PhysicalLayer.ANYSUSYID, PhysicalLayer.ANYSERIAL, pcktID);
				layer.write(command);
				layer.write(first);
				layer.write(last);
				layer.writePacketTrailer();
				...
				do {
					 ...
					 */
					{
						...
						if (pcktID == rcvpcktID) {
							...
							SmaSerial serial = new SmaSerial(
									(short)AbstractPhysicalLayer.getShort(data, 15),
							...
							if (current != null) {
								...
								for (int i = 41; i < data.length - 3; i += recordsize) {
									...
									Date datetime = new Date(AbstractPhysicalLayer.getInt(
											data, i + 4) * 1000L);
									...
									case GridMsTotW: // SPOT_PACTOT
										...
										logger.debug(strkW, lri.getCode(),
										        AbstractPhysicalLayer.tokW(value), datetime);
										break;
                                    ...
                                    case GridMsPhVphsC: // SPOT_UAC3
										...
										current.flags |= type.getValue();
										logger.debug(strVolt, lri.getCode(),
												AbstractPhysicalLayer.toVolt(value),
												datetime);
										break;
									...
									case GridMsAphsA_1: // SPOT_IAC1
										...
										logger.debug(strAmp, "SPOT_IAC1",
												AbstractPhysicalLayer.toAmp(value),
												datetime);
										break;
                                    ...
                                    case MeteringDyWhOut: // SPOT_ETODAY
										...
										logger.debug(strkWh, current + lri.getCode(),
												AbstractPhysicalLayer.tokWh(value64),
												datetime);
										break;
									...
									case MeteringTotOpTms: // SPOT_OPERTM
										...
										logger.debug(strHour, "SPOT_OPERTM",
												AbstractPhysicalLayer.toHour(value64),
												datetime);
										break;
		                            ...
		                            case NameplatePkgRev: //INV_SWVER
	                                    ...
	                                    current.swVersion = String.format("%c%c.%c%c.%02d.%s", '0'+(vMajor >> 4), '0'+(vMajor & 0x0F), '0'+(vMinor >> 4), '0'+(vMinor & 0x0F), vBuild, releaseType);
		                                current.flags |= type.getValue();
		                            ...
		                            case NameplateModel: //INV_TYPE
		                                ...
		                                {
		                                    int attribute = (int) (AbstractPhysicalLayer.getLong(data, i + idx) & 0x00FFFFFF);
